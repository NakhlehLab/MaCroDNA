...
if __name__=="__main__":
	...
	src_dir = "./macrodna_res_log_random_test_median"

	# read the actual objective functions 
	with open(os.path.join(src_dir, "macrodna_objvals_median.pkl"), "rb") as f:
		d = pickle.load(f)
	print(d)

	for bio in biop_sample:

		a = np.load(os.path.join(src_dir, bio)+"_random_medians.npy")
		print(a.shape)
		...
		ax.text(d[bio] - 0.01, ymax/2, score_label, fontsize=12)
		ax.set_xlabel("Score (median of the Pearson correlation values of pairs)")
		_ = ax.set_ylabel("Counts")
		ax.set_title(bio.replace("_", " (").replace("PAT", "Patient ")+")")
		plt.savefig(os.path.join(src_dir, bio)+"random_hist_median.pdf"
			, transparent=True)
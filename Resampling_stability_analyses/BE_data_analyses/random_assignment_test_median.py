...
class MaCroDNA:
    ...
    def cell2cell_assignment(self):
        ...
        all_scores_idx = np.argwhere(global_correspondence != 0)
        print(all_scores_idx.shape)
        all_scores_vals = corrs[all_scores_idx[:,0], all_scores_idx[:,1]]
        return tmp_result, tmp_result_tagged, sum(obj_scores), np.median(all_scores_vals.flatten())
...
class random_test:
    ...
    def assign(self):
        ...
        return s, np.median(np.concatenate(arrs))
...
def func(dna_src_dir, rna_src_dir, tgt_dir, d, N = int(1e6)):
    ...
    max_val = 0
    for i in range(N):
        sampled_s, sampled_median = random_assigner.assign()
        
        print(f"median of the pearson correlation values of pairs, random sample number {i}: {sampled_median}")
        if sampled_s > max_val:
            max_val = sampled_s
        random_scores.append(sampled_s)
        random_medians.append(sampled_median)


    random_scores = np.array(random_scores)
    random_medians = np.array(random_medians)
    
    np.save(os.path.join(tgt_dir, d+"_random_medians"), random_medians)
...
if __name__ == "__main__":
    ...
    tgt_dir = "./macrodna_res_log_random_test_median/"
    ...
    for d in biop_sample:
        ...
        tmp_result, tmp_result_tagged, obj_val, median_val = run_model.cell2cell_assignment()
        print(f"median of the preason correlation values of the assigned pairs {median_val}")
        dict_[d] = obj_val
        dict_median[d] = median_val
        tmp_result_tagged.to_csv(os.path.join(tgt_dir, d+"_cell2cell_assignment_indexed.csv"))

    with open(os.path.join(tgt_dir, "macrodna_objvals_median.pkl"), 'wb') as file_:
        pickle.dump(dict_median, file_)

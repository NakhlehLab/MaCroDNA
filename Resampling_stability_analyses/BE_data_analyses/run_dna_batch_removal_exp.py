...
class MaCroDNA_relaxed:
    ...
    def cell2cell_assignment(self):
        ...
        genes = set(self.dna_df.index).intersection(set(self.rna_df.index.to_list()))
        ...
        correspondent_dna_idx = np.argmax(corrs, axis=1)
        result_df = pd.DataFrame(list(zip([dna_cells[idx] for idx in correspondent_dna_idx], 
            rna_cells, 
            [1 for _ in range(len(rna_cells))])), 
...
def func(rand_seed, dna, rna, K, joint_cells, option, clus):
    ...
    if option == "relaxed":
        model_dat = MaCroDNA_relaxed(rna_copy, dna_copy)
    else:
...
if __name__ == "__main__":
    ...
    dna_src_dir = "/home/mae6/Bian_scripts_data/Seurat_preprocess/all_genes/"
    rna_src_dir = "/home/mae6/Bian_scripts_data/Seurat_preprocess/all_genes/"

    biop_sample = ["crc04", "crc10", "crc11"]
    options = ['constrained']
    clustering_settings = ["agglomerative_log", "agglomerative_raw", "intNMF_log", "intNMF_raw"]

    mother_dir = "/home/mae6/Bian_scripts_data/all_dna_removal_exp/"
    ...
    for d in biop_sample:
        for option in options:
            for clustering_setting in clustering_settings:
                ...
                dna = pd.read_csv(os.path.join(dna_src_dir,d+"_dna_raw.csv"), index_col=0)
                rna = pd.read_csv(os.path.join(rna_src_dir,d+"_rna_raw.csv"), index_col=0)

                # ========= read the cluster info =========
                cluster_dir = "/home/mae6/Bian_scripts_data/Clusters/"+clustering_setting
                ...
                # ========= path handle =========
                tgt_dir = f"dna_removal_exp_res_{clustering_setting}_{option}_{d}/"
                ...
                if option == "relaxed":
                    model_orig_dat = MaCroDNA_relaxed(rna, dna)
                else:
                ...
                processes = [pool.apply_async(func,
                    args = (rand_num, dna, rna, K_, joint_cells, option, clus)) for rand_num in range(N)]

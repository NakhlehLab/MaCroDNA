...
if __name__=="__main__":
	...
	for setting in clustering_settings:
		...
		for patient_biop in biop_sample:
			df = pd.read_csv(os.path.join(src_dir, f"dna_removal_exp_res_{setting}_{patient_biop}/dat.csv"), index_col=0)
		...
		df_ = pd.DataFrame({'Accuracy': accs, 'Data_label': accs_labels, 'Patient': patient_labels})
		plt.cla()
		plt.clf()
		g = sns.boxplot(data = df_, y = "Accuracy", x = "Patient", hue = "Data_label")
		g.set(title = setting)
		plt.tight_layout()
		plt.savefig(f"./{setting}_patients_dna_batch_removal_exp_boxplot.pdf")

		# ========= plot the violins showing the difference in the accuracies of the dropped batches before and after drop out =========
		plt.cla()
		plt.clf()
		df = pd.DataFrame({'Change in accuracy': diffs, 'Patient': diffs_patient_labels})
		g = sns.boxplot(data = df, y = 'Change in accuracy', x = 'Patient')
		g.set(title = setting)
		plt.tight_layout()
		plt.savefig(f"./{setting}_patients_dna_batch_removal_exp_changes_boxplot.pdf")
